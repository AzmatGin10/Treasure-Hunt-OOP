...
class Player(Entity):
    ...
    def loadout(self):
        ...
        if response == "Check Current Loadout":
            input(f"Your current Weapon is: {self.currentweapon.get_desc()}\nYour current Armour is: {self.currentarmour.get_desc()}")
            return self.loadout()
            
            
        if response == "Check Items":
            return self.loadout()
        if response == "Check Weapons":
            input(f"{[weapon.get_desc() for weapon in self.weapons]}")
            return self.loadout()
        elif response == "Check Armour":
            input(f"{[armour.get_desc() for armour in self.armours]}")
            return self.loadout()
        else:
            ""
class Enemy(Entity):
...
player.equip_armour(armour1)
player.equip_armour(armour2)
